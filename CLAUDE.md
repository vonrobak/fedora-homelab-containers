...
- **ADR-039 (Proposed):** Egress Baseline Scoping for High-Rotation Cloud-API Endpoints (amends ADR-030 P7) — crowdsec's CAPI is AWS-API-Gateway-fronted in eu-west-1 and rotates across the published pool, so per-IP allow-listing is a re-baseline treadmill (3 PRs in 48h). Scope crowdsec's AWS allowance to AWS-published eu-west-1 ranges via a deliberate offline sync (never hot-path); crowdsec-only carve-out gated by ≥3 benign re-baselines; sharpens the off-region exfil signal rather than discarding it. `infrastructure: []` unchanged. Not yet implemented
